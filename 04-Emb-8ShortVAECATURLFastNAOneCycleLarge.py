...
criteria_cc = nn.CrossEntropyLoss()


optimizer = torch.optim.AdamW(model.parameters())
dl = DataLoader(us, batch_size=batch_size, collate_fn=collate, shuffle=True)
best = float('inf')

scheduler = torch.optim.lr_scheduler.OneCycleLR(
    optimizer,
    max_lr=1e-2,                  # Max learning rate
    total_steps=len(dl) * 300,     # Total number of iterations
    pct_start=0.3,               # % of steps for increasing LR
    anneal_strategy='cos',       # Cosine annealing
    cycle_momentum=False         # AdamW doesn't use momentum
)

start = 0
# files = list(filter(lambda x: x.endswith('.pth') and x.startswith('emb-8ShortVAECATURLFastNAdamWOneCycle_') and 'best' not in x, os.listdir()))
# if len(files) > 0:
...
for epoch in range(start, epochs):
    epoch_loss = 0
    progress_bar = tqdm(dl, desc=f"Epoch {epoch+1}/{epochs}", leave = False)
    ok = 0
    fails = 0
    for x in progress_bar:
        ...
        t = torch.arange(y.shape[0], device=device)
        loss = (criteria_cc(y, t) + criteria_cc(y.T, t)) / 2
        ...
        #     if fails == 0:
        #         torch.save(x, 'x.pt')
        #         torch.save([model, optimizer, scheduler], 'fail_model.pt')
        #     fails += 1
        #     print('Failing!')
        #     if fails > 10:
        #         raise e
        optimizer.step()
        scheduler.step()
        # Update loss and tqdm bar
        epoch_loss += loss.item()
        acc = (y.argmax(dim=1) == torch.arange(y.shape[0], device=device)).sum().item() 
        ok = ok + acc
        acc = acc / len(client)
        progress_bar.set_postfix(loss=f"{loss.item():.4f} acc={acc:.4f}")
        
    progress_bar.close()
    c_loss = epoch_loss / len(dl)
    ok = ok / 1000000
    print(f"Epoch {epoch+1} Loss: {c_loss:.4f} acc={ok:.4f}")
    print(torch.cuda.mem_get_info())
    
    if c_loss < best:
        best = c_loss
        print('Saving best model!')
        torch.save(model.state_dict(), f'emb-8ShortVAECATURLFastNAdamWOneCycleLarge_best.pth')
    if (epoch + 1) % 10 == 0:
        torch.save({
            'epoch': epoch,
            ...
            'best': best
            }, f'emb-8ShortVAECATURLFastNAdamWOneCycleLarge_{(epoch + 1):03d}.pth')
        to_delete = f'emb-8ShortVAECATURLFastNAdamWOneCycleLarge_{(epoch + 1 - 10):03d}.pth'
