...
ema_model = torch.optim.swa_utils.AveragedModel(model, 
                                                multi_avg_fn=torch.optim.swa_utils.get_ema_multi_avg_fn(0.999)).to(device)
...
torch.save(ema_model.state_dict(), f'emb-8ShortVAECATURLFastNAdamWEMA_best_nn.pth')
momenta = {}
...
if len(momenta) > 0:
    ...
    for x in tqdm(dl):
        ...
        ema_model(actions, sku_add, sku_rm, sku_buy,
                sku_add_cat, sku_rm_cat, sku_buy_cat,
                kde_adds, kde_rms, kde_buys, kde_searchs, 
                kde_visits, query_search, url_visit)
        pass
...
torch.save(ema_model.state_dict(), f'emb-8ShortVAECATURLFastNAdamWEMA_best.pth')
